...
const (
	message = "Sign this message to access DeGate Exchange: %v with key nonce: %v"
)

func CreateAppKey(privateKey string, exchange string, keyNonce uint) (secretKey string, publicKeyX string, publicKeyY string, err error) {
	if !model.IsETHAddress(exchange) {
		err = errors.New("illegal exchange address")
		return
	}
	_, sign, err := signature.PrivateKeySign(privateKey, fmt.Sprintf(message, exchange, keyNonce))
	if err != nil {
		return
	}
...
func SignOrderRequest(privateKey string, exchange string, r *model.DGOrderParam) (s string, err error) {
	...
	feeTokenIdBig := new(big.Int).SetUint64(uint64(r.FeeToken.TokenId))
	maxFeeBipsBig, isBig := new(big.Int).SetString(r.FeeToken.Volume, 10)
	if !isBig {
		err = errors.New("error fee volume")
		return
	}
