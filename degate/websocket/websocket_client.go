...
func (c *WebSocketClient) HandlerMessage(message []byte) {
	...
	if payload["e"] == "kline" {
		...
		c.CheckToken(reportPayload.B, reportPayload.S1, reportPayload.F)
...
func (c *WebSocketClient) CheckToken(ids ...int) {
	...
	if len(noConfigIds) > 0 {
		...
		tokens, err := ec.GetTokenList(&model.TokenListParam{
			Ids: noConfigIds,
		})
