...
func (c *Client) PlaceOrder(param *model.OrderParam) (response *binance.NewOrderResponse, err error) {
	...
	if pairInfo.IsStable {
		price = util.GetEffectivePriceRound(price, pairInfo.IsStable, conf.OrderStabilityEffectiveDigitsGreaterThan1, conf.OrderStabilityEffectiveDigitsLessThan1)
	} else {
		price = util.GetEffectivePriceRound(price, pairInfo.IsStable, conf.OrderEffectiveDigitsGreaterThan10000, conf.OrderEffectiveDigitsLessThan10000)
	}
	...
	orderPrice := quoteQuantity.DivRound(quantity, 32)
	orderPriceDiff := decimal.Max(orderPrice, price).Sub(decimal.Min(orderPrice, price))
	log.Info("price:%v orderPrice:%v diff:%v", price.String(), orderPrice.String(), orderPriceDiff.String())
	if orderPriceDiff.GreaterThan(conf.OrderPriceDiff) {
		err = errors.New("order illegal")
		return
	}
	...
	balanceTokens = append(balanceTokens, r.FeeToken.TokenId)
...
func (c *Client) MarketOrder(param *model.OrderParam) (response *binance.NewOrderResponse, err error) {
	...
	depthResponse, err := c.GetDepth(&model.DepthParam{
		Symbol: param.Symbol,
		Limit:  2,
	})
	if err != nil {
	...
	if isBuy {
		...
		if pairInfo.IsStable {
			price = util.GetEffectivePriceRound(price.Mul(conf.MarketOrderBuyAdjustment), pairInfo.IsStable, conf.OrderStabilityEffectiveDigitsGreaterThan1, conf.OrderStabilityEffectiveDigitsLessThan1)
		} else {
			price = util.GetEffectivePriceRound(price.Mul(conf.MarketOrderBuyAdjustment), pairInfo.IsStable, conf.OrderEffectiveDigitsGreaterThan10000, conf.OrderEffectiveDigitsLessThan10000)
		}
		if param.Quantity > 0 {
			quoteQuantity = util.GetEffectiveVolume(quantity.Mul(price), conf.EffectiveDigits, quoteDecimalDigits, true)
			//quoteVolume = decimal.Min(quoteQuantity.Mul(conf.Pow10.Pow(decimal.NewFromInt32(quoteToken.Decimals))), conf.MaxVolume)
			quoteVolume = quoteQuantity.Mul(conf.Pow10.Pow(decimal.NewFromInt32(quoteToken.Decimals)))
		} else {
			quantity = util.GetEffectiveVolume(quoteQuantity.DivRound(price, 32), conf.EffectiveDigits, baseDecimalDigits, false)
			//baseVolume = decimal.Min(quantity.Mul(conf.Pow10.Pow(decimal.NewFromInt32(baseToken.Decimals))), conf.MaxVolume)
			baseVolume = quantity.Mul(conf.Pow10.Pow(decimal.NewFromInt32(baseToken.Decimals)))
		}
	} else {
		...
		price, err = decimal.NewFromString(depthResponse.Data.Bids[0][0])
		if err != nil {
			return
		}
		if pairInfo.IsStable {
			price = util.GetEffectivePriceRound(price.Mul(conf.MarketOrderSellAdjustment), pairInfo.IsStable, conf.OrderStabilityEffectiveDigitsGreaterThan1, conf.OrderStabilityEffectiveDigitsLessThan1)
		} else {
			price = util.GetEffectivePriceRound(price.Mul(conf.MarketOrderSellAdjustment), pairInfo.IsStable, conf.OrderEffectiveDigitsGreaterThan10000, conf.OrderEffectiveDigitsLessThan10000)
		}
		if param.Quantity > 0 {
...
func (c *Client) CancelAllOrders(includeGrid bool) (response *binance.Response, err error) {
	if err = c.CheckEddsaSign(); err != nil {
		return
	}
	...
	if !res.Success() {
		err = fmt.Errorf("cancelAllOrder failed: code=%d msg=%v", res.Code, res.Message)
	}
	return
}
